...
function scr_render3d_core(argument0,argument1,argument2,argument3,argument4,argument5=[0,0],argument6=false) {
	...
	repeat(sprite_get_number(model)/2) {
		...
		/*if overlayFlag && localLayerNumber==sprite_get_number(model)/2 {
			...
			if currentTriggers == 0 {shader_set(shd_greyscale)}
			draw_surface_ext(slice,targetLayerCenterX,targetLayerCenterY,xTiltSkew,yTiltSkew,0,color,1)
			if currentTriggers == 0 {shader_reset()}
		//}
